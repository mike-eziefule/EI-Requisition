document.addEventListener("DOMContentLoaded", function () {
    ...
    let lineItemIndex = document.querySelectorAll("#line-items .line-item").length;
    ...
    if (addItemButton) {
        ...
        console.warn("Add item button not found in the DOM.");
    }

    // Remove a line item dynamically
    ...
    if (submitChangesButton) {
        submitChangesButton.addEventListener("click", async function () {
            ...
            lineItems.forEach((item, index) => {
                ...
                // Remove `id` if it's null (for new items)
                if (!lineItemData.id) {
                    delete lineItemData.id;
                }
            ...
            requestFormData.append("requisition_input", JSON.stringify(requisitionData));
            if (attachment) {
    ...
    if (submitFormButton) {
        submitFormButton.addEventListener("click", async function () {
            ...
            if (!form) {
                console.error("Requisition form not found in the DOM.");
                alert("Form not found. Please refresh the page.");
                return;
            }
            ...
            for (let i = 0; formData.has(`line_items[${i}][item_name]`); i++) {
                requisitionData.line_items.push({
                    ...
                    quantity: parseInt(
                        formData.get(`line_items[${i}][quantity]`)
                    ),
        ...
        console.warn("Submit form button not found in the DOM.");
    }
